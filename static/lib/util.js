...
var util; util = {
    ...
    mobile: _ => {
        ...
        return m;
    }, // UA sniffing: https://stackoverflow.com/questions/3514784/what-is-the-best-way-to-detect-a-mobile-device
    cookie: (id, val, date) => {
    ...
    load_script: (url, load) => {
        var script_node = document.createElement('script');
        script_node.onload = load;
        script_node.src = url;
    ...
    rand_id: (length, chars = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ') => {
        var result = '';
    ...
    dec_hex: n => {
        return util.lpad(String(n.toString(16)), 2, '0');
    },
    rgb_hex: (r, g, b, hash = false) => {
        return `${hash ? '#' : ''}${util.dec_hex(parseInt(r))}${util.dec_hex(parseInt(g))}${util.dec_hex(parseInt(b))}`;
    },
    hex_rgb: (hex, hash = false) => {
        if (hash) hex = hex.substring(1);
