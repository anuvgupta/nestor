...
module.exports = {
        ...
        return {
            link: (node, client, value, transitional, next) => {
                ...
                if (value != 'null') {
                    ...
                    if (exec_code) m.main.set_node_shortcut(value, node._id.toString(), node.core_id, node_type, "exec", 'link', exec_code, node.data.hasOwnProperty('icon') ? node.data.icon : null, node.name);
                }
                next();
            },
    ...
    api: {
        ...
        setup_shortcut: (m, log, api_args, resolve) => {
            ...
            if (value) {
                var exec_code = m.main.get_driver_field_metadata(node_type, 'link', 'editor-options')['shortcut-exec'];
                if (exec_code) m.main.set_node_shortcut(value, node._id.toString(), node.core_id, node_type, "exec", 'link', exec_code, icon, node.name);
