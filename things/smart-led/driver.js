/* nestor thing server driver (smart-led) */

const thing_type = "smart-led";
        ...
        get_pattern_code: (m, log, api_args, resolve) => {
            if (api_args) {
                ...
                if (pattern_id && node_id) {
                    m.db.get_node_info(node_id, null, `_api[${thing_type}][get_pattern_code]`, node_info => {
                                ...
                                if (ws_client || mq_client) {
                                    m.db.get_user_data(user_id, "color_patterns", pattern_list => {
                                        ...
                                        if (pattern_list) {
                                            ...
                                            if (pattern_string) {
                                                log(`driver[${thing_type}.get_pattern_code] sending pattern to node ${node_id}: ${pattern_id}:${pattern_string}`);
                    ...
                    setTimeout(_ => {
                        ...
                        if (!audio_flag) {
                            setTimeout(_ => {
                                ...
                                log(`driver[${thing_type}.play_initial] sending currently playing data to node ${node_id}`);
                            }, 200);
                        }
                        resolve(true);
                    }, 50);
                    log(`driver[${thing_type}.play_initial] sending audio react mode update to node ${node_id}`);
                }, 200);
            }
        }
    }
