...
var api = {
    node_drivers: {},
    load_node_drivers: _ => {
        fs.readdirSync(path.join(__dirname, "../../things")).forEach(type_id => {
            if (type_id[0] != '.' && type_id[0] != '_') {
                var module_driver_def = require(`../../things/${type_id}/driver.js`);
                var meta_data = JSON.parse(fs.readFileSync(`../things/${type_id}/meta.json`, { encoding: 'utf8', flag: 'r' }));
                if (!meta_data) meta_data = {};
                meta_data.web = {
                    client: fs.existsSync(`../things/${type_id}/client.js`),
                    views: fs.existsSync(`../things/${type_id}/views.block`)
                };
    ...
    init_node_driver: (node_id, type, node = null) => {
        ...
        if (m.main.node_drivers[type].api.hasOwnProperty('__create')) { // used to say '__spawn', is that right? probably not
    ...
    get_driver_reset_interval: (node_type) => {
        if (m.main.node_drivers.hasOwnProperty(node_type) && m.main.node_drivers[node_type].hasOwnProperty('data')) {
            ...
            if (node_metadata.hasOwnProperty('node') && node_metadata.thing.hasOwnProperty('reset')) {
                return node_metadata.thing.reset;
            }
        }
        return -1;
    },
